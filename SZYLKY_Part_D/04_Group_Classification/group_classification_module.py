...
BALANCED_THRESHOLD_COMPONENTS = GROUP["balanced_metric_components"]
SUPPORTED_THRESHOLD_METRICS = {"f1", "accuracy", "balanced_accuracy", "precision", "recall", "gmean", "balanced4"}
...
def _is_split_artifact_feature(column: str) -> bool:
    ...
    return "year" in lowered or "source" in lowered or "来源文件" in column or "鏉ユ簮" in column
...
def _feature_columns(x: pd.DataFrame) -> list[str]:
    ...
    policy = str(GROUP.get("feature_policy", "rank_temporal")).strip().lower()
    percentile_cols = [c for c in candidates if c.endswith("_pct_rank")]
    temporal_cols = [c for c in candidates if c.startswith(("temporal_", "future_"))]
    if percentile_cols and policy in {"rank_temporal", "rank_only"}:
        return percentile_cols + ([] if policy == "rank_only" else temporal_cols)
    if percentile_cols and policy == "rank_temporal_raw_signal":
        keywords = tuple(str(k).lower() for k in GROUP.get("raw_feature_keywords", ()))
        raw_signal_cols = [
            c
            for c in candidates
            if not c.endswith("_pct_rank")
            and not c.startswith(("temporal_", "future_"))
            and any(k in c.lower() for k in keywords)
        ]
        return list(dict.fromkeys(percentile_cols + raw_signal_cols + temporal_cols))
    if percentile_cols and policy == "all_nonleaky":
        return candidates
    return candidates
...
def _build_model(random_state: int, y: pd.Series | np.ndarray | None = None) -> Pipeline:
...
def _threshold_score(metrics: dict, threshold_metric: str) -> float:
    if threshold_metric == "balanced4":
        values = np.array([metrics[name] for name in BALANCED_THRESHOLD_COMPONENTS], dtype=float)
        if np.any(np.isnan(values)):
            return float("nan")
        return float(len(values) / np.sum(1.0 / np.clip(values, 1e-12, None)))
    return metrics[threshold_metric]
...
def _best_threshold(y_true: pd.Series, y_prob: np.ndarray, threshold_metric: str = "f1") -> tuple[float, dict]:
    ...
    for thr in THRESHOLD_GRID:
        ...
        score = _threshold_score(metrics, threshold_metric)
        if np.isnan(score):
            continue
...
def predict_groups(
    ...
    classifier = payload.get("classifier", classifier_name(models))
