...
class TemporalModelWrapper(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int = TEMPORAL["hidden_dim"], model_type: str = TEMPORAL["model_type"]):
        super().__init__()
        self.model_type = model_type.lower()
        if self.model_type == "transformer":
            self.encoder = TransformerTemporalEncoder(input_dim, hidden_dim)
        else:
                                                                      
            self.encoder = GRUTemporalEncoder(input_dim, hidden_dim, dropout=TEMPORAL["dropout"])
...
def _pairwise_cosine(x: np.ndarray, top_k: int = 12, min_sim: float = 0.55) -> pd.DataFrame:
    ...
    xn = x / norms
    rows = []
    top_k = max(1, min(int(top_k), len(xn) - 1))
    chunk_size = int(TEMPORAL["similarity_chunk_size"])
    for start in range(0, len(xn), chunk_size):
        stop = min(start + chunk_size, len(xn))
        scores = xn[start:stop] @ xn.T
        for local_i, row_scores in enumerate(scores):
            i = start + local_i
            row_scores[i] = 0.0
            if top_k < len(row_scores):
                candidate_idx = np.argpartition(-row_scores, top_k)[:top_k]
                idx = candidate_idx[np.argsort(-row_scores[candidate_idx])]
            else:
                idx = np.argsort(-row_scores)
            for j in idx:
                if row_scores[j] >= min_sim:
                    rows.append((i, int(j), float(row_scores[j])))
...
def _predict_next_adjacency(
    ...
    reverse = edges.rename(columns={"Provider_src": "Provider_dst", "Provider_dst": "Provider_src"})
    edges = pd.concat([edges, reverse], ignore_index=True)
    edges = edges[edges["Provider_src"] != edges["Provider_dst"]]
    edges = edges.groupby(["Provider_src", "Provider_dst"], as_index=False)["weight"].max()
    return _limit_max_degree(edges, max_degree)


def _edge_list_to_dense_graph(providers: list[str], edges: pd.DataFrame) -> pd.DataFrame:
    ...
    if not edges.empty:
        for _, row in edges.iterrows():
            src = provider_index.get(str(row["Provider_src"]))
            dst = provider_index.get(str(row["Provider_dst"]))
            if src is None or dst is None:
                continue
            mat[src, dst] = max(mat[src, dst], float(row["weight"]))
...
def build_temporal_outputs(
    ...
    model = TemporalModelWrapper(input_dim=f_dim, hidden_dim=TEMPORAL["hidden_dim"], model_type=TEMPORAL["model_type"])
    ...
    if load_from_train_model:
        ...
        with log_stage(f"训练时序模型 {split}"):
            ...
            model.train()
            total_epochs = int(TEMPORAL["epochs"])
    ...
    with log_stage(f"构建时序关系图 {split}"):
        ...
        rel_edges = _predict_next_adjacency(
            providers,
            emb_np,
            business,
        ...
        if write_dense_graph:
            if len(providers) > int(TEMPORAL["dense_graph_provider_limit"]):
                log_line(
                    f"Skipping dense graph CSV: provider_count={len(providers)} would create a very large matrix; edge list was saved",
                    tag="SKIP",
                )
            else:
                relation_graph = _edge_list_to_dense_graph(providers, rel_edges)
