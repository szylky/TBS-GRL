...
def _split_source_csvs(dataset_root: Path, split: str) -> list[Path]:
    ...
    if csvs:
        return csvs
    direct = dataset_root / f"DMEPOS_{split}.csv"
...
def _is_dmepos_flat(df: pd.DataFrame) -> bool:
    return {"Rfrg_NPI", "Year"}.issubset(df.columns)
...
def _dmepos_risk_maps(
    train_claims: pd.DataFrame,
    categorical_cols: list[str],
    smoothing: float = PREPROCESSING["dmepos_target_smoothing"],
    ...
    for provider, group in grouped:
        ...
        for col in categorical_cols:
            ...
            if frequency_map:
                ...
                row[f"{col}_rare_share"] = float((freq <= PREPROCESSING["rare_category_frequency_threshold"]).mean()) if len(freq) else 0.0
...
def _smoothed_risk_maps(
    frame: pd.DataFrame,
    cols: list[str],
    global_rate: float,
    smoothing: float = PREPROCESSING["generic_target_smoothing"],
